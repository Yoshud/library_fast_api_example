...
async def test_create_book_with_title(client):
    book_data = {"id": "200001", "book_title": {"title": "New Book", "author": "Author Name"}}
...
async def test_create_book_with_title_id(client):
    ...
    book_data1 = {"id": "200002", "book_title": {"title": "Existing Book", "author": "Existing Author"}}
    ...
    book_data2 = {"id": "200003", "book_title_id": title_id}
...
async def test_create_book_duplicate_id(client):
    book_data = {"id": "200004", "book_title": {"title": "T", "author": "A"}}
...
async def test_update_borrowers(client):
    ...
    update_data = {"update_borrowers_map": {"200005": "100001"}}
    ...
    # Return book
    return_data = {"update_borrowers_map": {"200005": None}}
...
async def test_bulk_update_borrowers_borrow_and_return(client):
    ...
    update_data = {"update_borrowers_map": {"200010": None, "200011": "100011"}}
    response = await client.put("/api/books/", json=update_data)
    assert response.status_code == 200
...
async def test_bulk_update_borrowers_rollback_on_error(client):
    ...
    update_data = {"update_borrowers_map": {"200012": "100012", "999999": "100012"}}
    response = await client.put("/api/books/", json=update_data)
    assert response.status_code == 404
