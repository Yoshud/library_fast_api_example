from app.repositories.book_copy_repository import BookCopyRepository, BookCopyRepositoryException
...
class BookManagerServiceException(Exception):
    pass

class BookManagerBookCopyNotFoundException(BookManagerServiceException):
...
class BookManagerUserNotFoundException(BookManagerServiceException): pass

class BookManagerAlreadyBorrowedException(BookManagerServiceException):
...
class BookManagerServiceUnknownIntegrityException(BookManagerServiceException): pass
...
class BookManager:
    ...
    async def create_book_copy_with_book_title(self,
            db: AsyncSession,
        ...
        async with optional_transaction(db):
            ...
            except BookCopyRepositoryException as e:
                raise e
        ...
        async with optional_transaction(db):
            ...
            for copy_id in requested_ids:
                if copy_id not in existing_copies:
                    raise BookManagerBookCopyNotFoundException(copy_id)
                ...
                if new_user_id is not None:
                    if copy.user_id is not None:
                        raise BookManagerAlreadyBorrowedException(copy_id, copy.user_id)
            ...
            try:
                ...
                if pg_error_code == FOREIGN_KEY_VIOLATION_PG_ERROR_CODE:
                    raise BookManagerUserNotFoundException from e
                raise BookManagerServiceUnknownIntegrityException from e
